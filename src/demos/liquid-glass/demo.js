...
class GlassCard extends Entity {
  ...
  refract() {
    ...
    cctx.drawImage(wp, sx, sy, w * mag * s, h * mag * s, -12, -12, w * s + 24, h * s + 24);
    ...
    const strip = 20;
    // left, right
    cctx.drawImage(wp, (px - bend) * s, py * s, strip * s, h * s, 0, 0, strip * s, h * s);
    cctx.drawImage(
      wp,
      (px + w - strip + bend) * s,
      py * s,
    ...
    // top, bottom
    cctx.drawImage(wp, px * s, (py - bend) * s, w * s, strip * s, 0, 0, w * s, strip * s);
    cctx.drawImage(
      wp,
      px * s,
      (py + h - strip + bend) * s,
  ...
  render(r) {
    ...
    r.beginPath();
    r.roundRect(3, 3, w - 6, h * 0.4, [this.radius - 3, this.radius - 3, 18, 18]);
    r.fill(gloss);
    ...
    r.fill(fillGrad);
    r.fillCircle(track.x + track.w * this.sliderValue, track.y + track.h / 2, 9, '#ffffff');
    r.flush();
  }
}
...
window.addEventListener('pointermove', (e) => {
  ...
  if (card.draggingSlider) {
    const local = card.worldToLocal(e.clientX - rect.left, e.clientY - rect.top);
